...
public class TimePlatformController
{
    ...
    public void UpdatePlatform(PlatformState state, PlatformView view, float deltaTime, float time )
    {
        ...
        Vector3 newVelocity = _calculatePlatformMovement(state, view, deltaTime, time);
        _calculatePassengerMovement(state, view, state.velocity);

        
        state.velocity = newVelocity;
        state.prevPosition = state.position;
        
        _movePassengers(state, true);
        state.position += state.velocity;
        view.transform.position = state.position;
        _movePassengers(state, false);

        view.viewRoot.position = state.prevPosition;
    }
    
    private void _movePassengers(PlatformState state, bool beforeMovePlatform)
    {
    ...
    private Vector3 _calculatePlatformMovement(PlatformState state, PlatformView view, float deltaTime, float time)
    {
        ...
        if (state.percentBetweenWaypoints >= 1)
        {
            ...
            if(view.cycleMode == PlatformState.CycleMode.YOYO && state.fromWaypointIndex >= state.globalWayPoints.Length - 1)
            {
                state.fromWaypointIndex = 0;
                System.Array.Reverse(state.globalWayPoints);
            }
        }
        return newPos - state.position;
    }
    
    private void _calculatePassengerMovement(PlatformState state, PlatformView view, Vector3 velocity)
    {
